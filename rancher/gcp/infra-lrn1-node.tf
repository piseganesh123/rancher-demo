...
resource "google_compute_address" "learner_1_node_address" {
  name = "learner-1-node-ipv4-address"
  address_type = "EXTERNAL"
}

# GCP compute instance for creating a single node workload cluster
resource "google_compute_instance" "learner_1_node"{ #"quickstart_node"
  depends_on = [
  ...
  name         = "${var.prefix}-learner_1-node"
  ...
  network_interface {
    network = "default"
    access_config {
      nat_ip = google_compute_address.learner_1_node_address.address
    }
  }
  ...
  metadata_startup_script = templatefile(
    "${path.module}/files/userdata_quickstart_node.template",
    {
      ...
      public_ip        = google_compute_address.learner_1_node_address.address
