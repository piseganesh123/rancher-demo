...
resource "google_compute_address" "learner11_node_address" {
#  count = 2
#  name = format("%s%s","node-ipv4-address",(count.index+2))
  name = "learner11-node-ipv4-address"
  address_type = "EXTERNAL"
}

# GCP compute instance for creating a single node workload cluster
resource "google_compute_instance" "learner11_node"{ #"quickstart_node"
#  depends_on = [
...
#  name         = "${var.prefix}-learner11-node"
  name = "learner11-node"
...
#  machine_type = "e2-medium"
  ...
  network_interface {
    network = "default"
    access_config {
      nat_ip = google_compute_address.learner11_node_address.address
    }
  }
  ...
  metadata_startup_script = templatefile(
    ...
    {
      register_command = module.rancher_common.learner11_custom_cluster_command #custom_cluster_command
      public_ip        = google_compute_address.learner11_node_address.address
    }
  )
